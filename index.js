...
`}function h(r){return r.map(f).join("")}let m=new c(".gallery-item a",{captionsData:"alt",captionDelay:250});const n={gallery:document.querySelector(".gallery"),searchMenu:document.querySelector(".search-menu"),searchBtn:document.querySelector(".search-btn"),searchInput:document.querySelector(".search-input"),loader:document.querySelector(".loader")};n.searchMenu.addEventListener("submit",r=>{r.preventDefault();const o=n.searchInput.value.trim();o&&(n.gallery.innerHTML="",n.searchInput.value="",n.loader.style.display="inline-block",y(o).then(s=>{s.length===0?(u.error({message:"Sorry, there are no images matching your search query. Please try again!",position:"topRight"}),n.gallery.innerHTML=""):(n.gallery.innerHTML=h(s),m.refresh())}).catch(s=>{console.error(s),n.gallery.innerHTML=""}).finally(()=>{n.loader.style.display="none"}))});
