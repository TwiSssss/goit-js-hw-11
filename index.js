...
`}function h(r){return r.map(y).join("")}let m=new c(".gallery-item a",{captionsData:"alt",captionDelay:250});const n={gallery:document.querySelector(".gallery"),searchBtn:document.querySelector(".search-btn"),searchInput:document.querySelector(".search-input"),loader:document.querySelector(".loader")};n.searchBtn.addEventListener("click",r=>{r.preventDefault();const o=n.searchInput.value.trim();o&&(n.searchInput.value="",n.loader.style.display="inline-block",f(o).then(s=>{s.length===0?u.error({message:"Sorry, there are no images matching your search query. Please try again!",position:"topRight"}):(n.gallery.innerHTML=h(s),m.refresh())}).catch(s=>(console.error(s),[])).finally(()=>{n.loader.style.display="none"}))});
